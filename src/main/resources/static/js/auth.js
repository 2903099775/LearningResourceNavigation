// 全局错误处理，处理401/403错误
(function() {
    // 不触发弹窗的接口白名单（这些接口自己会处理未登录逻辑）
    const NO_POPUP_URLS = [
        '/api/user/profile',   // checkLoginStatus 主动检查登录状态
        '/api/public/auth/login', // 登录接口本身
    ];
        ...
        return originalFetch.apply(this, arguments)
            .then(response => {
                if (response.status === 401 || response.status === 403) {
                    // 检查是否在白名单中，白名单接口不弹窗（自己处理）
                    const urlStr = typeof url === 'string' ? url : (url && url.url ? url.url : '');
                    const isWhitelisted = NO_POPUP_URLS.some(u => urlStr.includes(u));
                    if (!isWhitelisted) {
                        // 非白名单接口遇到 401/403 时弹出登录弹窗
                        showLoginModal();
                    }
                    // 注意：直接透传 response，不 reject！让各接口自己的 .then 继续处理
                    // 这样 checkLoginStatus 的清除 token 逻辑可以正常执行
                }
                return response;
            });
    };
            ...
            .then(data => {
                ...
                if (data.code === 200) {
                    if (data.data && data.data.token) {
                        ...
                        // 存储完整用户信息（含 vipExpireDate），避免 VIP 状态切换页面后丢失
                        const userData = {
